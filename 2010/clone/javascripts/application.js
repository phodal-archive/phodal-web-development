...
$(".reply").live('click',function(){
  var val = $('#content').val();
  var user_name =  $(this).parents('.content-title').find(".publish_name").text();
  var user_id = $(this).parents('.content-title').find('.user_id').val();
  $('#to_user_id').val(user_id);
  if ( /^(回复).*:/.test(val))
     $('#content').val(val.replace(/^(回复).*:/, '回复'+user_name+':'));
  else
     $('#content').val('回复'+user_name+': ' + val);
	$('html,body').animate({scrollTop: $('.discuss-form').offset().top-200}, 200);
}); 
...
function copy_link(target){
  if(window.clipboardData){
    window.clipboardData.setData("Text",$(target).val());
  } else {
    alert("对不起，目前此功能只支持IE，请按 Ctrl + C 直接复制！");
    $(target).select();  
  }
}
